...
export async function getProgressData(period: Period = "30d"): Promise<ProgressData> {
  ...
  const [oneRM, weeklyVolume, muscleDistribution, workoutFrequency, stats] = await Promise.all([
    fetchOneRM(since),
    fetchWeeklyVolume(since, weeks),
    fetchMuscleDistribution(since),
    fetchWorkoutFrequency(since, weeks),
    fetchOverallStats(since),
  ]);

  return { oneRM, weeklyVolume, muscleDistribution, workoutFrequency, stats };
}

async function fetchOneRM(since: string | null): Promise<ExerciseOneRM[]> {
  let query = supabase
    .from("sets")
    .select(
...
async function fetchMuscleDistribution(since: string | null): Promise<MuscleGroupDistribution[]> {
  let query = supabase
    .from("sets")
...
async function fetchOverallStats(since: string | null) {
