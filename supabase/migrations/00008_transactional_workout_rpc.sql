-- Transactional workout write APIs.
-- Consolidates workout + sets writes behind RPC boundaries.
...
alter table public.sets
  add constraint sets_set_number_positive
  check (set_number > 0);
...
create or replace function public.delete_workout_cascade(target_workout_id uuid)
returns boolean
language plpgsql
security invoker
set search_path = ''
...
begin
  ...
  if deleted_count = 0 then
    raise exception 'Workout not found or delete window expired';
  end if;
