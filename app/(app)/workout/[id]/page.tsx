...
export default async function WorkoutDetailPage({
  ...
  const { data: workout } = await supabase
    .from("workouts")
    .select("id, name, started_at, finished_at, notes, sets(id, set_number, weight, reps, rpe, duration_s, exercise_id, exercises(id, name, muscle_group, equipment))")
    .eq("id", id)
    .single();
  ...
  const exerciseMap = new Map<string, { id: string; name: string; muscle_group: string | null; equipment: string | null }>();
  const sets = (workout.sets ?? []).map((set) => {
    // Supabase повертає масив через Relationships, але для FK це завжди 1 елемент
    const exerciseArr = set.exercises as unknown as { id: string; name: string; muscle_group: string | null; equipment: string | null } | null;
